...
웹 도구를 개발할 때는 보안에 신경 써야 한다. AI가 생성한 코드에는 SQL 인젝션, XSS(크로스 사이트 스크립팅), 인증 우회 등의 취약점이 포함될 수 있다. 기능 구현이 완료되면 Claude Code에게 보안 점검을 요청한다:

> 프로젝트 전체를 포괄적으로 보안 점검하고, 문제가 있으면 수정해줘

Claude Code가 코드를 분석하여 SQL 인젝션 가능성, 사용자 입력 미검증, 환경 변수 노출, CORS 설정 문제 등을 찾아내고 수정해 준다. 이 과정을 기능을 추가할 때마다 반복하는 것이 좋다.
