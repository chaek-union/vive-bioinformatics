...
## 13.4 Paper Writing Assistant로 논문 작성하기

13.3절에서 소개한 Paper Writing Assistant(https://research.pnucolab.com)를 사용하여 실제로 논문을 작성하는 과정을 살펴본다. 웹사이트에 접속하면 5단계 워크플로우를 순서대로 안내받을 수 있다.

### 1단계: 형식 선택

웹 도구에서 논문 유형(원저, 리뷰, 레터 등)을 선택한다. 투고할 학술지의 가이드라인에 맞는 형식을 고른다. 원저 논문이라면 IMRAD 구조가 자동으로 적용된다.

### 2단계: 문헌 선별

**연구자가 직접 읽고 선별한 논문**을 입력한다. 12장에서 구축한 Co-scientist의 Literature Reviewer 에이전트가 후보 논문을 검색하고 요약해 줄 수 있지만, 최종 선별은 연구자의 몫이다. 선별한 논문의 BibTeX을 입력하면 인용 관리가 자동으로 처리된다.

### 3단계: 관점 정의

연구의 핵심 관점과 주장을 모국어로 입력한다. 예를 들어:

> "기존 bulk RNA-seq 분석은 종양 미세환경의 세포 이질성을 포착하지 못한다. 우리는 단일세포 분석으로 종양 침윤 T세포의 하위 집단을 규명하고, 이 중 exhausted CD8+ T세포가 면역관문억제제 반응과 상관관계가 있음을 보여준다."

이 한 문단이 논문 전체의 방향을 결정한다.

### 4단계: 개요 작성

각 섹션(Introduction, Methods, Results, Discussion)의 핵심 포인트를 입력한다. "이것만은 반드시 포함해야 한다"는 내용을 미리 정리해 두면, AI가 빠뜨리거나 임의로 추가하는 것을 방지할 수 있다.

### 5단계: 텍스트 생성

앞서 입력한 문헌, 관점, 개요를 바탕으로 AI가 영어 원고를 생성한다. 연구자가 제공한 자료만 사용하도록 설계되어 있어, 허위 인용 위험이 크게 줄어든다. 생성이 완료되면 투명성 보고서도 함께 만들어진다.

### Figure 생성은 Claude Code로

논문의 figure는 Paper Writing Assistant가 아닌 Claude Code로 생성하는 것이 효과적이다.
...
- **Paper Writing Assistant 실습**: 웹 도구에서 5단계 워크플로우를 따라 실제 논문 작성
