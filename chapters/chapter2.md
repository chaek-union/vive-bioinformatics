...
두 번째 방법을 권장하는 이유는, Github에서 저장소를 만들 때 README, .gitignore, 라이선스 파일 등을 자동으로 추가할 수 있어 초기 설정이 편리하기 때문이다.

물론 반드시 원격 저장소로 Github을 쓸 필요는 없다. Bitbucket, Gitlab, Phorge 등 다른 서비스도 활용 가능하다. 다만 이 책에서는 가장 널리 사용되는 Github을 기준으로 설명한다.
