...
```bash
claude mcp add biorxiv -s user -- uvx biorxiv-mcp
claude mcp add pubmed -s user -- uvx pubmed-mcp
claude mcp add fetch -s user -- npx -y @anthropic/mcp-server-fetch
```
...
> .claude/settings.json에 Notification Hook을 추가해줘. 분석이 완료되거나 중요한 발견이 있을 때 Discord webhook으로 알림을 보내는 셸 스크립트를 만들어줘. webhook URL은 .env 파일에서 읽어오게 해줘.
    ...
    "Notification": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .claude/hooks/notify.sh \"$CLAUDE_NOTIFICATION\""
          }
        ]
      }
    ],
...
알림 스크립트(`notify.sh`)는 메시지를 받아 webhook으로 전송한다. Discord, Slack, Zulip 모두 webhook URL에 JSON을 POST하는 방식으로 동작하므로, 플랫폼에 따라 JSON 형식만 약간 다르다.
...
데이터가 준비되면 AI 팀원들이 **동시에** 작업을 시작한다. Data Analyst는 데이터를 분석하고, Literature Scout은 관련 문헌을 조사한다.

> @data-analyst data/ 디렉토리의 scRNA-seq 데이터를 분석해줘. QC (미토콘드리아 유전자 비율, 유전자 수, UMI 수 기준 필터링), 정규화 (scran), 고변동 유전자 선택, PCA, UMAP, Leiden 클러스터링을 수행해줘. 각 단계의 QC 그래프를 figures/ 에 저장하고, 결과를 results/preprocessed.h5ad로 저장해줘.

> @literature-scout NSCLC tumor microenvironment의 면역세포 구성에 관한 최근 1년간 bioRxiv 프리프린트와 PubMed 논문을 검색해줘. 특히 T세포 하위 집단 분류 기준과 마커 유전자 목록을 정리해줘. 결과를 references/literature-review.md에 저장해줘.

분석이 진행되는 동안 Notification Hook이 Discord 채널에 업데이트를 보낸다:
...
분석이 진행되는 동안 알림이 전송된다:

```
🔬 [Data Analyst] 가설 1 검증 — exhausted CD8+ T세포 비율: 반응군 12.3% vs 비반응군 4.1% (p=0.003)
🔬 [Data Analyst] 가설 3 검증 — pseudotime 분석 완료, M1→M2 전환 중간 상태 3개 확인
🧬 [Pathway Analyst] GO enrichment 완료 — T cell exhaustion 관련 term 유의 (FDR < 0.001)
```
...
보고서가 완성되면, 13장에서 배운 5단계 워크플로우에 따라 논문을 작성한다. 연구자가 관점과 개요를 제공하고, AI가 영어 텍스트를 생성하는 역할 분담이다.

> 논문 초안을 작성해줘. 형식은 IMRAD (Introduction, Methods, Results, Discussion)이야. 내 관점: "단일세포 분석으로 NSCLC 종양 미세환경의 면역세포 이질성을 규명하고, exhausted CD8+ T세포 비율이 면역관문억제제 반응의 바이오마커가 될 수 있음을 제시한다." references/literature-review.md에 정리된 논문만 인용해줘. 내가 제공한 내용만 사용해줘.
...
알림 시스템은 연구팀의 소통 도구에 맞게 설정한다. 어떤 플랫폼을 사용하든 원리는 동일하다: HTTP POST 요청으로 메시지를 전송한다.
...
> .claude/hooks/notify.sh를 플랫폼에 맞게 업데이트해줘. .env에 NOTIFICATION_PLATFORM (discord, slack, zulip 중 하나)과 WEBHOOK_URL을 설정하면 자동으로 해당 플랫폼 형식으로 전송하도록 해줘.

### 알림 시점과 내용

어떤 시점에 어떤 내용을 알릴지도 중요하다. 너무 자주 알리면 노이즈가 되고, 너무 드물면 진행 상황을 파악할 수 없다.

| 시점 | 알림 내용 | Hook 이벤트 |
|------|----------|------------|
| 분석 단계 완료 | "QC 완료 — N개 세포 통과" | Stop |
| 주요 발견 | "p < 0.05 결과 발견" | Stop |
| 오류 발생 | "메모리 부족으로 클러스터링 실패" | Stop |
| 파일 생성 | "figure 저장: figures/umap.png" | PostToolUse |
| 전체 완료 | "분석 파이프라인 완료" | Stop |
