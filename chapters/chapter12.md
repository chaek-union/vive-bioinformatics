...
KEGG(Kyoto Encyclopedia of Genes and Genomes)는 유전자와 대사 경로 정보를 제공하는 데이터베이스이다. 유전자가 어떤 생물학적 경로에 관여하는지 조회할 때 필수적으로 사용된다. KEGG MCP 서버를 만들면 Claude가 직접 경로 정보를 검색할 수 있다.
...
MCP 서버를 처음부터 직접 작성할 필요는 없다. Claude Code에게 요청하면 된다:
...
이미 사용하고 있는 Python 스크립트가 있다면, 그것을 MCP 서버로 변환하는 것이 가장 빠르다. 기존 함수에 `@mcp.tool()` 데코레이터를 추가하고 docstring을 달면 된다.

## 12.5 커스텀 에이전트 정의

Claude Code는 `.claude/agents/` 디렉토리에 커스텀 에이전트를 정의할 수 있다. 각 에이전트는 **특정 역할과 도구 접근 권한**이 있으며, 마크다운 파일로 정의한다.
...
매 세션 시작 시 현재 연구 진행 상황을 자동으로 Claude에게 전달해 준다. 연구가 며칠에 걸쳐 진행될 때, 매번 "지난번에 어디까지 했지?"를 설명할 필요가 없어진다.
