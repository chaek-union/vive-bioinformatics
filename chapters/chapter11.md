...
Claude Code가 대화 과정에서 **스스로 학습한 내용**을 기록하는 파일이다. 예를 들어 데이터 분석 에이전트가 "이 데이터셋은 배치 효과가 크므로 Harmony 보정이 필요하다"는 사실을 발견하면 MEMORY.md에 기록한다. 이후 같은 데이터를 다시 분석할 때는 이 메모리를 읽고 처음부터 Harmony 보정을 적용한다. 사용자가 일일이 지시하지 않아도 에이전트가 경험을 축적하고 행동을 개선하는 것이다.
