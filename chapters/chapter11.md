...
- **`-s user`**: 전역 설정에 추가한다. 모든 프로젝트에서 사용 가능. 프로젝트별로만 사용하려면 `-s project`를 지정한다.
- **`--`** 뒤의 명령: MCP 서버를 실행하는 명령이다. `uvx`는 Python 패키지, `npx`는 Node.js 패키지를 실행한다.

> **중요**: MCP 서버를 추가한 후에는 **Claude Code를 재시작해야 한다**. MCP 서버는 Claude Code가 시작할 때 함께 실행되므로, 새로 추가한 서버를 인식하려면 재시작이 필요하다.

생명정보학 MCP 서버처럼 여러 프로젝트에서 공통으로 사용할 서버는 전역 설정(`-s user`)에, 특정 프로젝트에만 필요한 서버는 프로젝트 설정(`-s project`)에 추가하는 것이 좋다.
...
- **설정 방법**: 터미널에서 `claude mcp add` 명령으로 서버 등록
  - `-s user`(전역) 또는 `-s project`(프로젝트별)로 범위 지정
