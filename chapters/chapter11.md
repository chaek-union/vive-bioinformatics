...
에이전트가 단순한 도구와 구별되는 또 다른 특징은 **메모리(Memory)**다. 컨텍스트는 대화가 끝나면 사라지지만, 메모리는 대화를 넘어 **영구적으로 유지**된다.

Claude Code의 메모리는 **CLAUDE.md** 파일이다. 이 파일에 프로젝트의 규칙, 코딩 컨벤션, 분석 파이프라인의 세부 사항 등을 기록해두면, 새로운 대화를 시작해도 Claude Code가 이전 맥락을 기억한다. 컨텍스트가 대화 중 단기 기억이라면, CLAUDE.md는 대화를 넘어 유지되는 장기 기억이다.

더 중요한 것은 에이전트가 **스스로 메모리를 업데이트**할 수 있다는 점이다. 분석 과정에서 새로운 패턴을 발견하면 CLAUDE.md에 기록하고, 다음 작업에서 이를 참조한다. 예를 들어 데이터 분석 에이전트가 "이 데이터셋은 배치 효과가 크므로 Harmony 보정이 필요하다"는 사실을 발견하면 CLAUDE.md에 기록한다. 이후 같은 데이터를 다시 분석할 때는 이 메모리를 읽고 처음부터 Harmony 보정을 적용한다. 이처럼 경험을 축적하고 행동을 개선하는 것이 에이전트의 **자가 진화**다.
